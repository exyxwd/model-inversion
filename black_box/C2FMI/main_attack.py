...
def post_de(latent_in, generator, target_model, target_label, idx):
    if len(idx) == 1:  # Handle single index case
        x = latent_in[idx[0]].unsqueeze(0)  # Add batch dimension
    else:
        x = latent_in[idx]
...
def inversion_attack(generator, target_model, embed_model, p2f, target_label, latent_in, attack_step, optimizer,
                     lr_scheduler, face_shape, img_size, input_is_latent, top_x=1):  # Added top_x parameter
    ...
    for i in pbar:
        ...
        if (i+1) % attack_step == 0:
            with torch.no_grad():
                ...
                for j in range(min(top_x, iii.size(0))):  # Select top-x indices
                    idx.append(iii[j].item())
...
if __name__ == '__main__':
    ...
    for target_label in range(init_label, fina_label):
        ...
        # Pass the top_x parameter to inversion_attack
        inversion_attack(g_ema, target_model, embed_model, p2f, target_label, latent_in, step, optimizer,
                         lr_scheduler, face_shape, img_size, input_latent, top_x)  