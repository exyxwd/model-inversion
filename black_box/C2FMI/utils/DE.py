...
class Optimizer(object):
    ...
    def gen_img(self, x):
        x = torch.tensor(x, dtype=torch.float32).to(self.device)  # Ensure float32
        with torch.no_grad():
...
class DE_c2b_5_bin(object):
    ...
    def extend(self):
        ...
        print(f"Extending population: batch_size={self.batch_size}, popsize={self.popsize}, num_lack={num_lack}")
        
        if self.batch_size == 1:  # Special case for batch_size=1
            for _ in range(num_lack):
                u = self.pop[0] + np.random.randn(*self.pop[0].shape) * 0.1  # Add small random noise
        ...
        uu = np.concatenate(tmp, axis=0).astype(np.float32)  # Ensure float32
    ...
    def step(self, step_i):
        ...
        children = np.concatenate(children, axis=0).astype(np.float32)  # Ensure float32
    ...
    def get_img(self, num_imgs, only_best=False):
        ...
        print(f"Generation {i}, max fitness: {np.max(fitness_par)}")
